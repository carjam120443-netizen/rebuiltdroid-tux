...
for t in curl xorriso unsquashfs mksquashfs file find mount umount; do command -v "$t" >/dev/null 2>&1 || { echo "Missing required tool: $t" >&2; exit 1; }; done
rm -rf "$WORK" "$OUT"
...
[[ -f "$ANDROID_SYSTEM/framework/framework.jar" ]] || { echo "Android system image has no framework.jar" >&2; sudo umount "$MOUNT"; exit 5; }
mkdir -p "$ANDROID_SYSTEM/etc/rebuiltdroid-tux"
printf '%s\n' 'RebuiltDroid Tux' 'Base: Android-x86 Android 10 x64' 'Filesystem: system.img inside system.sfs' > "$ANDROID_SYSTEM/etc/rebuiltdroid-tux/BUILD_INFO"
